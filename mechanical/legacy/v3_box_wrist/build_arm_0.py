...
def Arm0_GetComponents(p):
    ...
    wr=p['wrist_shaft_diameter']/2; wi=p['wrist_bearing_id']/2; wo=p['wrist_bearing_od']/2; wb=p['wrist_bearing_width']
    # J5 fixed U carrier: thick bridge, two box-section arms and separate opposite seat.
    j5plate=Shape_Orient(Solid_CutHoles(Shape_Ring(49,22,6,(0,0,-3)),m['fixed_holes_xy'],3.4,-3,6),(w,-58,0),(0,1,0))
    roll=Shape_Box(12,187,40,4,-108,-20).cut(Shape_Cylinder(8.2,12,(4,0,0),(1,0,0)))
    roll=roll.fuse(Shape_Box(112,10,40,4,-108,-20)).fuse(Shape_Box(12,53,40,104,-108,-20)).fuse(j5plate)
    roll=roll.fuse(Shape_Box(56,10,40,4,69,-20))
    j5seat=Shape_Ring(23,16.15,12,(w,57,0),(0,1,0)).fuse(Shape_Ring(23,8,2,(w,69,0),(0,1,0)))
    roll=roll.fuse(j5seat).cut(Shape_Cylinder(8.5,10,(w,69,0),(0,1,0)))
    cap5=Shape_Ring(23,8.5,2,(w,55,0),(0,1,0))
    roll=Shape_DrillY(roll,Pattern_GetCircle(20,3,90),2.3,57,5,ox=w)
    cap5=Shape_DrillY(cap5,Pattern_GetCircle(20,3,90),3.4,55,2,ox=w)
    Component_Add('J5_BoxYokeCarrier','roll',roll,assembly='J5')
    Component_Add('J5_OppositeBearing','roll',Shape_Ring(wo,wi,wb,(w,60,0),(0,1,0)),'bearing','J5','GEOMETRIC PLACEHOLDER 15x32x9')
    Component_Add('J5_BearingRetainer','roll',cap5,assembly='J5')
    j5hub=Shape_Ring(19,5,4,(0,-58,0),(0,1,0)).fuse(Shape_Ring(24,5,4,(0,-54,0),(0,1,0))).fuse(Shape_Ring(wr,5,6,(0,-50,0),(0,1,0)))
    j5hub=Shape_DrillY(j5hub,[(x,-z) for x,z in m['output_holes_xy']],4.5,-58,8)
    Component_Add('J5_ShortOutputHub','pitch',j5hub,'metal','J5')
    shaft=Shape_Cylinder(wr-.1,24,(0,47,0),(0,1,0)).fuse(Shape_Cylinder(22,3,(0,44,0),(0,1,0)))
    shaft=Shape_DrillY(shaft,Pattern_GetCircle(18,3,0),3.4,44,3)
    Component_Add('J5_OppositeShortStub','pitch',shaft,'metal','J5')
    # J6 motor sits between short thick pitch cheeks; front bearing supports its output.
    side1=Shape_Ring(28,wr+.2,6,(0,-50,0),(0,1,0)).fuse(Shape_Box(38,6,52,0,-50,-26))
    side1=side1.cut(Shape_Cylinder(wr+.2,6,(0,-50,0),(0,1,0)))
    side1=Shape_DrillY(side1,[(x,-z) for x,z in m['output_holes_xy']],4.5,-50,6)
    side2=Shape_Ring(28,wr+.2,6,(0,47,0),(0,1,0)).fuse(Shape_Box(38,6,52,0,47,-26))
    side2=side2.cut(Shape_Cylinder(wr+.2,6,(0,47,0),(0,1,0)))
    side2=Shape_DrillY(side2,Pattern_GetCircle(18,3,0),3.4,47,6)
    front=Shape_Orient(j4plate,(fl,0,0),(1,0,0))
    tabs=[Shape_Box(6,12,32,fl-3,-50,-16),Shape_Box(6,15,32,fl-3,38,-16)]
    pitch=Shape_Merge([side1,side2,front]+tabs)
    # Tapered annular nose: output hub remains free inside; bearing has a real axial shoulder.
    nose=Part.makeCone(43,23,8,App.Vector(fl+3,0,0),App.Vector(1,0,0)).cut(Part.makeCone(37,17,8,App.Vector(fl+3,0,0),App.Vector(1,0,0)))
    nose=nose.fuse(Shape_Ring(23,16.15,12,(fl+13,0,0),(1,0,0))).fuse(Shape_Ring(23,8,2,(fl+11,0,0),(1,0,0)))
    pitch=pitch.fuse(nose)
    cap6=Shape_Ring(23,8.5,3,(fl+25,0,0),(1,0,0))
    for yy,zz in Pattern_GetCircle(20,3,90):
        pitch=pitch.cut(Shape_Cylinder(1.15,5,(fl+20,yy,zz),(1,0,0)))
        cap6=cap6.cut(Shape_Cylinder(1.7,3,(fl+25,yy,zz),(1,0,0)))
    Component_Add('J6_PitchBoxCarrier','pitch',pitch,assembly='J6')
    Component_Add('J6_OutputBearing','pitch',Shape_Ring(wo,wi,wb,(fl+14,0,0),(1,0,0)),'bearing','J6','GEOMETRIC PLACEHOLDER 15x32x9')
    Component_Add('J6_BearingRetainer','pitch',cap6,assembly='J6')
    j6hub=Shape_Ring(19,5,8).fuse(Shape_Ring(wr-.1,5,22,(0,0,8)))
    j6hub=Solid_CutHoles(j6hub,m['output_holes_xy'],4.5,0,8)
    j6hub=Shape_Orient(j6hub,(0,0,0),(1,0,0))
    tool=Shape_Ring(20,5,5,(30,0,0),(1,0,0)).fuse(Shape_Ring(12,6,p['Tool']-35,(35,0,0),(1,0,0)))
    Component_Add('J6_ShortOutputHub','tool',j6hub.fuse(tool),'metal','J6','ONE-PIECE metal hub / tool datum envelope; machining route provisional')
...
def Arm0_BuildDocument(cfg=None,save=True):
    ...
    for k,v in dict(BaseHeight=180,UpperArm=320,Forearm=290,Wrist=60,Flange=35,Tool=90,MotorCount=6).items():
